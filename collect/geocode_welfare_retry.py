...
def main():
    ...
    def work(r):
        ...
        if done["n"] % 500 == 0:
            print(f"  진행 {done['n']}/{len(fails)} · 복구 {done['ok']}")
        if res:
            r[xi], r[yi] = res
            done["ok"] += 1
    ...
    print(f"복구 {done['ok']}건 · 최종 커버리지 {filled}/{len(data)} ({filled/len(data)*100:.1f}%) · {time.time()-t0:.0f}s")
