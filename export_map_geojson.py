...
FAC = [('pharmacy','ph',1),('clinic','cl',1),('emergency','em',1),('school','sc',1),('store','st',0),
       ('park','pk',1),('bus','bs',1),('cctv','cc',1),('ev','ev',1)]
...
def score_props():
    ...
    rows = con.execute(f"""SELECT s.adm_cd, sd.SIDO_NM||' '||sg.SIGUNGU_NM||' '||s.adm_nm AS full_nm, s.adm_nm, s.cohort, s.pop_total,
       s.NLI, s.grade, s.score_D1,s.score_D2,s.score_D3,s.score_D4,s.score_D5,s.score_D6,s.score_D7, {cnt}
       FROM nli_scores s LEFT JOIN sido sd ON substr(s.adm_cd,1,2)=sd.SIDO_CD LEFT JOIN sgg sg ON substr(s.adm_cd,1,5)=sg.SIGUNGU_CD""").df()
    props = {}
    for _, r in rows.iterrows():
        ...
        for c in rows.columns:
            ...
            v = r[c]
            d[c] = (round(v,1) if isinstance(v,float) and (c.startswith('score') or c=='NLI')
                    else (int(v) if isinstance(v,float) and v==v else v))
        props[r['adm_cd']] = d
    return props
