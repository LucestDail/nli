...
def main():
    ...
    df = con.execute("SELECT * FROM nli_scores").df()
    df = df[df["pop_total"] > 0].copy()

    # ── 균형성: 7도메인 점수 표준편차(낮을수록 올라운드) ──
    df["balance_std"] = df[DOM_COLS].std(axis=1)
    df["profile"] = pd.cut(df["balance_std"], [0, 12, 22, 100],
    ...
    con.register("res", df)
    con.execute("CREATE OR REPLACE TABLE nli_scores AS SELECT * FROM res")  # balance_std/profile 추가 반영
    print("\n저장: nli_blindspots.csv, nli_deficiencies.csv, nli_scores(+balance_std,profile)")
