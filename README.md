...
공공데이터 표준데이터를 **읍면동(행정동) 단위**로 융합해, "어디가 살기 좋은 동네인가"를 8개 도메인으로 정량화한 지수 + 정적 웹 대시보드.
...
| D2 교육·보육 | 초중등학교 · **학원(상가 세분)** |
| D3 생활편의·상업 | **생활편의 상가**(음식·소매·수리·숙박) |
| D4 문화·여가·체육 | 도시공원 · **여가 상가**(예술·스포츠) |
| D5 교통·이동 | 버스정류소 |
| D6 안전 | CCTV |
| D7 환경·기후 | 전기차충전소 |
| D8 복지·돌봄 | 사회복지시설 (VWorld 지오코딩 85% 커버) |

*지역특성 축*: 인구밀도 · 인구 · 고령/유소년비중 · 아파트비율 · **노후주택비율**(1999년 이전).
...
6탭 SPA(지도 기본·홈·순위·비교·맞춤설정·통계):

- **지도**: 지표별 백분위 색칠 · 사각지대/인구대비 모드 · 시설 포인트(확대 시) · 클릭 상세
- **맞춤설정**: 페르소나 10종 + 객관가중 프리셋, 실시간 가중치 슬라이더 → 지도/순위 즉시 반영
