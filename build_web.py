...
function fillTransit(p){
  ...
  fetch(u).then(r=>r.json()).then(j=>{const e=document.getElementById('transitInfo');if(!e)return;
    if(j&&j.found&&j.durationMin!=null){e.innerHTML=`🚇 <b>${commuteBase}</b>→여기 대중교통 <b style="color:var(--ocean)">약 ${j.durationMin}분</b>`+(j.transfers?` · 환승 ${j.transfers}`:'')+(j.estimatedFareKrw?` · ₩${j.estimatedFareKrw.toLocaleString()}`:'')+` <span style="font-size:10px">(오전9시 기준)</span>`;}
