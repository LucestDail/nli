...
<meta property="og:description" content="전국 3,559개 읍면동 · 9도메인 32지표 + 아파트 실거래가. 가성비 좋은 동네를 찾아보세요.">
...
<meta name="twitter:image" content="https://lucestdail.github.io/nli/assets/og.png">
 ...
 *{box-sizing:border-box}
 ...
 .wrap{max-width:1220px;margin:0 auto;padding:14px 30px 30px}
 ...
 aside{width:var(--aside-w);background:#faf8f4;border-right:1px solid var(--line);overflow-y:auto;padding:11px 13px;flex-shrink:0}
 ...
 .scb{flex:1 1 auto;min-width:0}
 .statgrid td[title]{cursor:pointer}
 #dsScatterBox{position:relative}#dsScatterBox circle{cursor:pointer}
 ...
 .mapsum{position:absolute;left:calc(var(--aside-w) + 14px);bottom:16px;z-index:600;background:rgba(255,255,255,.95);backdrop-filter:blur(6px);border:1px solid var(--line);border-radius:12px;box-shadow:0 6px 20px -8px rgba(20,30,40,.3);padding:10px 13px;font-size:12px;max-width:230px}
 ...
 .scrollv{max-height:440px;overflow-y:auto;border:1px solid var(--line2);border-radius:10px}
 ...
 .statgrid table.heat{width:100%;table-layout:fixed}
 ...
 .findlist{display:flex;flex-direction:column;gap:6px;max-height:255px;overflow:auto}
...
<div id="app">
 ...
 <div id="methodModal" class="rmodal" style="display:none" onclick="if(event.target===this)closeMethod()"><div class="rmbox" style="width:600px;max-width:100%;max-height:88vh">
   ...
   <div class="mbody">
     ...
     <ul>
       ...
       <li><b>가중은 선택(정량화)</b> — 균등가중은 관행적 선택이나, 대안 가중 13종·무작위 500회 검증에서 순위상관 0.98~1.00·등급변동 중앙 9%로 등급은 <b>가중에 강건</b>합니다. 다만 밀도 상관은 <b>모든 가중에서 0.86~0.90</b>으로 재가중해도 벗어나지 못해, 위의 동일유형 비교로 대응합니다.</li>
       <li><b>개수 ≠ 용량</b> — 대형·소형 시설을 개수로 셉니다(병상·정원 미반영).</li>
       <li><b>공급측 관점</b> — 시설 존재·근접을 보며, 품질·이용·수요는 별도입니다.</li>
       <li><b>복지·돌봄</b>은 지오코딩 약 96% 커버(일부 결측 하향 편향 가능).</li>
     </ul>
     <h4>그래서 어떻게 쓰면 되나</h4>
   ...
   <div id="findLanding">
     ...
     <div class="grid2" style="gap:14px;margin-bottom:12px">
       <div class="card"><div class="bhd"><h3 style="margin:0">가성비 동네</h3>
         ...
         <div class="muted" style="margin:3px 0 6px">살기지수 높고 아파트값 낮은 · 클릭 → 상세</div>
       ...
       <div class="card"><div class="bhd"><h3 style="margin:0">지역 순위</h3>
         ...
         <div class="muted" style="margin:3px 0 6px">종합 살기지수 상위 · 클릭 → 상세</div>
         <div id="findRankTop" class="findlist"></div></div>
     </div>
     <div class="card findsearch">
       <h3 style="margin:0 0 4px">동네 검색 · 맞춤 추천</h3>
       <div class="muted" style="margin-bottom:6px">동네 이름으로 찾거나, 조건을 넣어 나에게 맞는 동네를 추천받으세요.</div>
       ...
       <div class="fld" style="margin-top:11px">조건으로 맞춤 추천 <span class="muted" style="font-weight:400;text-transform:none;letter-spacing:0">— 가구유형 가중치</span></div>
       <div class="seg" id="recHouseSeg" style="margin:5px 0 9px"><button data-h="일반" class="on">일반</button><button data-h="육아">육아</button><button data-h="1인">1인가구</button><button data-h="고령">고령</button><button data-h="반려">반려동물</button></div>
       ...
       <button class="btn on" style="margin-top:11px;padding:9px 24px;width:100%;font-size:14.5px" onclick="recRun()">맞춤 동네 추천받기</button>
     </div>
   </div>
 ...
 <div class="view" id="v-diag" style="display:none"><div class="wrap inswrap">
   <h2>지역 생활여건 진단 <span class="muted" id="diagUnitLabel" style="font-size:14px;font-weight:400">— 전국 읍면동</span></h2>
   <div class="sub" id="diagSub" style="margin-bottom:12px">읍면동별 <b>취약·강점 도메인</b>과 <b>사각지대</b>(인구 1만+ 인데 전국 하위 20%) 진단 · <b>좌측 표에서 동을 클릭하면 우측 리포트가 갱신</b>됩니다. 상단에서 <b>시군구</b> 단위(지자체 229곳 집계)로 전환할 수 있습니다.</div>
   <div class="flex" style="margin-bottom:12px">
     <span id="diagUnitSeg" style="display:inline-flex;gap:4px;margin-right:2px"><button class="btn on" data-u="dong" onclick="setDiagUnit('dong')">동</button><button class="btn" data-u="sgg" onclick="setDiagUnit('sgg')">시군구</button></span>
     <input type="text" id="diagSearch" placeholder="동·지자체 검색 (예: 동춘3동, 강남구)" style="width:220px" autocomplete="off">
     <select id="diagSido"></select>
     <select id="diagSort"></select><button class="btn on" id="diagStatBtn" onclick="openDiagStat()" style="display:none">통계 분석</button><span id="diagSelInfo" class="muted"></span>
...
function updateViewSummary(){const el=document.getElementById('mapSummary');if(!el||!map)return;
  ...
  for(const f of F){const p=f.properties;if(p.clat==null||(p.pop_total||0)<=0)continue;
    if(p.clat<b.getSouth()||p.clat>b.getNorth()||p.clon<b.getWest()||p.clon>b.getEast())continue;
    const v=nliW(p);if(v==null)continue;n++;sum+=v;
    if(!best||v>best.v)best={nm:p.adm_nm,v:v};if(!worst||v<worst.v)worst={nm:p.adm_nm,v:v};}
  if(n<2){el.style.display='none';return;}
  el.style.display='block';
  el.innerHTML='<div class="mstop">현재 화면 <b>'+n.toLocaleString()+'</b>개 동 · 평균 종합 <b style="color:'+color(sum/n)+'">'+(sum/n).toFixed(1)+'</b></div>'
    +'<div class="msrow" style="color:#2f6b4e">▲ '+best.nm+' <b>'+best.v+'</b></div>'
    +'<div class="msrow" style="color:#b0603f">▼ '+worst.nm+' <b>'+worst.v+'</b></div>';}
function dongTip(p){const g=gradeOf(p),nv=nliW(p);
  const ds=DKEYS.map(d=>[d,p['score_'+d]]).filter(x=>x[1]!=null).sort((a,b)=>a[1]-b[1]);
  const weak=ds.length?DOMINFO[ds[0][0]][0]+SHORT[ds[0][0]]:'',strong=ds.length?DOMINFO[ds[ds.length-1][0]][0]+SHORT[ds[ds.length-1][0]]:'';
  const coh=p.nli_coh!=null?' · 유형내 상위 '+Math.max(1,Math.round(100-p.nli_coh))+'%':'';
  const metric=(mMetric&&mMetric!=='NLI'&&p['score_'+mMetric]!=null)?'<div class="dtiprow">'+METRICS[mMetric]+' <b style="color:'+color(p['score_'+mMetric])+'">'+Math.round(p['score_'+mMetric])+'</b></div>':'';
  return '<div class="dtiphd"><b>'+(p.adm_nm||'')+'</b> <span class="g" style="background:'+GC[g]+'">'+g+'</span></div>'
    +'<div class="dtiprow">종합 <b style="color:'+color(nv)+'">'+nv+'</b>'+coh+'</div>'+metric
    +(strong?'<div class="dtiprow" style="color:#2f6b4e">강점 '+strong+'</div>':'')
    +(weak?'<div class="dtiprow" style="color:#b0603f">취약 '+weak+'</div>':'')
    +'<div class="dtipft">클릭 → 상세</div>';}
...
function showTab(v){
  ...
  ['map','rec','diag'].forEach(x=>{const el=document.getElementById('v-'+x);if(!el)return;
    ...
    if(on&&x!=='map'){el.style.flex='1 0 auto';el.style.overflow='visible';el.style.animation='none';void el.offsetWidth;el.style.animation='vin .3s cubic-bezier(.22,1,.36,1)';}});
...
const PT_TYPES={em:['응급의료','#c0392b'],ph:['약국','#2f6b4e'],cd:['어린이집','#e07a9c'],sc:['학교','#5b6bd0'],lb:['도서관','#4a7bd0'],pk:['공원','#6f9e86'],sp:['체육시설','#3aa88a'],pg:['주차장','#7a8290'],ev:['전기차','#d4a056'],wf:['사회복지','#8e5db0'],sr:['경로당','#c47c52']};
...
function mRedraw(){if(!layer)return;layer.setStyle(mStyle);mLegend();updateViewSummary();
...
function mLegend(){const el=document.getElementById('legend');
  ...
  if(mMode==='coh'){el.innerHTML='<div class="muted" style="margin-bottom:4px;font-size:11px">같은 도농유형(도시·도농·농촌) 내 백분위</div>'+[[80,'상위 80–100'],[65,'65–80'],[50,'50–65'],[35,'35–50'],[20,'20–35'],[0,'하위 0–20']].map(b=>`<div><i style="background:${color(b[0])}"></i>${b[1]}</div>`).join('');return}
...
function dongToDiag(adm){const f=F.find(x=>x.properties.adm_nm===adm);if(!f)return;
  ...
  try{closeDongModal();}catch(e){}
  showTab('insight');
  setTimeout(function(){try{setDiagUnit('sgg');}catch(e){}const sb=document.getElementById('diagSearch');if(sb){sb.value=ps[1];}try{pickDiag(key);}catch(e){}
    const el=document.getElementById('v-diag');if(el)el.scrollTop=0;},40);}
...
function recCard(p,rank,score){
  ...
  return '<div class="card" style="padding:15px">'
    ...
    +'<div class="flex" style="gap:6px"><button class="btn on" onclick="openDongModal(\''+p.adm_nm+'\')">상세</button><button class="btn" onclick="shareDong(\''+p.adm_nm+'\',this)">공유</button></div></div>';
}
...
function findDongHTML(p){const doms=DOMS.map(d=>[d,p['score_'+d]]).filter(x=>x[1]!=null).sort((a,b)=>b[1]-a[1]);
  ...
  return '<div class="card"><div class="flex" style="justify-content:space-between;align-items:flex-start;gap:12px">'
    ...
    +(pct?'<div style="margin-top:8px;display:inline-block;background:rgba(180,124,82,.12);color:var(--terra);font-weight:800;font-size:13px;border-radius:999px;padding:5px 14px">전국 상위 '+pct+'%</div>':'')+(p.nli_coh!=null?'<div style="margin-top:8px;margin-left:6px;display:inline-block;background:rgba(63,143,168,.12);color:var(--ocean);font-weight:800;font-size:13px;border-radius:999px;padding:5px 14px" title="같은 도농 유형끼리 공정 비교">동일 유형 내 상위 '+Math.max(1,Math.round(100-p.nli_coh))+'%</div>':'')+'</div>'
    ...
    +(_sgg?'<div style="margin-top:14px;padding-top:13px;border-top:1px solid var(--line)"><a onclick="dongToDiag(\''+p.adm_nm+'\')" style="color:var(--ocean);cursor:pointer;font-weight:600;font-size:13px">이 동네가 속한 <b>'+_sgg+'</b> 생활여건 진단 보기 →</a></div>':'')+'<div class="flex" style="margin-top:14px;gap:8px"><button class="btn on" onclick="goDetail(\''+p.adm_nm+'\')">지도에서 자세히</button><button class="btn" onclick="addCmp(\''+p.adm_nm+'\')">⊕ 비교에 담기</button></div></div>';}
function recRun(){
...
function renderDiag(){return diagUnit==='dong'?renderDiagDong():renderDiagSgg();}
function setDiagUnit(u){
  diagUnit=u;diagSel=null;diagCmp=[];
  var seg=document.getElementById('diagUnitSeg');if(seg)seg.querySelectorAll('[data-u]').forEach(function(b){b.classList.toggle('on',b.dataset.u===u);});
  var so=document.getElementById('diagSort');
  so.innerHTML=(u==='dong'
    ?'<option value="nli">취약(종합 낮은) 순</option><option value="nli_desc">우수(종합 높은) 순</option><option value="blind">사각지대 도메인 많은 순</option><option value="coh">동일유형 하위 순</option><option value="pop">인구 많은 순</option>'
    :'<option value="nli">취약(평균지수 낮은) 순</option><option value="nli_desc">우수(평균지수 높은) 순</option><option value="blind">사각지대 많은 순</option><option value="blind_asc">사각지대 적은 순</option><option value="pop">인구 많은 순</option><option value="dongs">관할 동 많은 순</option>');
  diagSort='nli';so.value='nli';
  if(!so._w){so._w=1;so.onchange=function(){diagSort=so.value;renderDiag();};customSelect(so);}
  var sb=document.getElementById('diagStatBtn');if(sb)sb.style.display=(u==='sgg'?'inline-block':'none');
...
function renderDiagDong(){
  ...
  if(!sf.options.length){sf.innerHTML='<option value="">전국</option>'+[...new Set(SP.map(function(p){return p.sido;}))].filter(Boolean).sort().map(function(s){return '<option>'+s+'</option>';}).join('');sf.onchange=function(){diagSel=null;renderDiag();};customSelect(sf);}
  var dsr=document.getElementById('diagSearch');if(dsr&&!dsr._w){dsr._w=1;dsr.oninput=function(){diagSel=null;renderDiag();};}
  var nat={};DKEYS.forEach(function(d){var vs=SP.filter(function(p){return (p.pop_total||0)>0&&p['score_'+d]!=null;}).map(function(p){return p['score_'+d];});nat[d]=vs.reduce(function(a,b){return a+b;},0)/(vs.length||1);});
  var all=SP.filter(function(p){return (p.pop_total||0)>0;});
  var natRank=all.slice().sort(function(a,b){return (nliW(a)||0)-(nliW(b)||0);}),rankOf=new Map();natRank.forEach(function(p,i){rankOf.set(p.adm_nm,i+1);});
  var fsido=sf.value;var list=all.filter(function(p){return !fsido||p.sido===fsido;});
  var dq=((document.getElementById('diagSearch')||{}).value||'').trim();if(dq)list=list.filter(function(p){return (p.full_nm||'').includes(dq)||(p.adm_nm||'').includes(dq);});
  var bc=function(p){return DKEYS.reduce(function(n,d){return n+(isBlind(p,d)?1:0);},0);};
  var S={nli:function(a,b){return (nliW(a)||0)-(nliW(b)||0);},nli_desc:function(a,b){return (nliW(b)||0)-(nliW(a)||0);},blind:function(a,b){return bc(b)-bc(a)||(nliW(a)||0)-(nliW(b)||0);},coh:function(a,b){return (a.nli_coh==null?101:a.nli_coh)-(b.nli_coh==null?101:b.nli_coh);},pop:function(a,b){return (b.pop_total||0)-(a.pop_total||0);}};
  list.sort(S[diagSort]||S.nli);
  document.getElementById('diagCount').textContent=list.length.toLocaleString()+'개 동';
  ...
  shown.forEach(function(p){var nv=nliW(p)||0,gr=gradeOf(p),wd=DKEYS.map(function(d){return [d,(p['score_'+d]||0)-nat[d]];}).sort(function(a,b){return a[1]-b[1];})[0],nb=bc(p),coh=p.nli_coh!=null?Math.max(1,Math.round(100-p.nli_coh)):null,sd=(p.full_nm||'').split(' ')[1]||'';
    h+='<tr onclick="pickDiagDong(\''+p.adm_nm+'\')" style="cursor:pointer'+(p.adm_nm===diagSel?';background:#e4f0e8;box-shadow:inset 3px 0 0 var(--ocean)':'')+'">'
      ...
      +'<td style="text-align:left"><b>'+p.adm_nm+'</b> <span class="muted" style="font-size:11px">'+sd+'</span></td>'
...
function openDongModal(adm){const f=F.find(x=>x.properties.adm_nm===adm);if(!f)return;const p=f.properties;
  ...
  const _sgg=(p.full_nm||'').split(' ')[1]||'';
  body+=(_sgg?'<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--line)"><a onclick="dongToDiag(\''+adm+'\')" style="color:var(--ocean);cursor:pointer;font-weight:600;font-size:13px">이 동네가 속한 <b>'+_sgg+'</b> 생활여건 진단 보기 →</a></div>':'')+'<div class="flex" style="margin-top:12px"><button class="btn on" style="width:100%;padding:11px" onclick="closeDongModal();goDetail(\''+adm+'\')">지도에서 보기 →</button></div>';
  document.getElementById('dongModalBody').innerHTML=body;
...
/* Esc로 열린 모달 닫기(a11y) */
document.addEventListener('keydown',function(e){if(e.key!=='Escape')return;var M=[['routeModal',closeRoute],['diagStatModal',closeDiagStat],['dongModal',closeDongModal],['methodModal',closeMethod],['srcModal',closeSrc]];for(var i=0;i<M.length;i++){var el=document.getElementById(M[i][0]);if(el&&getComputedStyle(el).display!=='none'){M[i][1]();break;}}});
let _dstat=null;
...
function openDiagStat(){
  ...
  document.getElementById('diagStatBody').innerHTML=
    '<div class="flex" style="justify-content:space-between;align-items:baseline;gap:10px;flex-wrap:wrap"><h2 style="margin:0">'+(single?gs[0].sido+' '+gs[0].sgg:gs.length+'개 지자체 비교 분석')+'</h2><div class="muted">관할 동 '+dongs.length+'개 · 표 편차=전국 지자체 평균 대비 · 셀·점에 커서=상세 · 하단 관할 동 표에서 동 클릭=지도</div></div>'
    +'<div class="statgrid">'
    +'<div class="sc"><h4>도메인 프로파일 <span>레이더 · 평균 점수(백분위)</span></h4><div class="scb"><div style="text-align:center">'+radar+'</div>'+legend+'</div></div>'
    +'<div class="sc"><h4>종합지수 · 사각지대</h4><div class="scb"><div class="fld" style="font-size:11px">평균 종합지수</div>'+nliBars+'<div class="fld" style="font-size:11px;margin-top:12px">사각지대(동×도메인)</div>'+blBars+'</div></div>'
    +'<div class="sc"><h4>함께 갖춰지는 도메인 <span>관할 동 기준 · 두 도메인이 같이 좋은 정도(초록=동반)</span></h4><div class="scb">'+dc+'</div></div>'
    +'<div class="sc"><h4>지역 특성 ↔ 생활여건 <span>인구밀도·고령 등이 도메인과 연관되는 정도(초록=정비례)</span></h4><div class="scb">'+rh+'</div></div>'
    +'<div class="sc"><h4>지자체 × 도메인 점수 <span>평균 점수 · 초록 높음</span></h4><div class="scb">'+hm+'</div></div>'
    +'<div class="sc wide"><h4>관할 동 산점도 <span>축 선택 · 점=동(커서 올리면 정보) · 색=지자체</span></h4><div class="flex" style="gap:8px;margin-bottom:8px;font-size:12px;flex-shrink:0">X '+sel('dsX',xo,'dens')+' Y '+sel('dsY',yo,'NLI')+' <span id="dsReg" class="muted"></span></div><div class="scb"><div id="dsScatterBox"></div></div></div>'
    +'</div>'
    +'<div class="statgrid">'
    +'<div class="sc"><h4>도메인 편차 표 <span>선택 지자체 · 전국 지자체 평균 대비 · 행별 최고 초록</span></h4><div class="scb">'+t+'</div></div>'
    +'<div class="sc wide"><h4>관할 동 전체 <span>'+dongs.length+'개 · 종합지수 낮은 순 · <b style="color:#b0603f">●사각</b> 사각지대('+totalBl+'건) · 스크롤 · 클릭→지도</span></h4><div class="scb">'+dt+'</div></div>'
    +'</div>';
...
function renderDStatScatter(){
  ...
  let g='<svg viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="xMidYMid meet" style="width:100%;height:auto;max-height:320px;display:block"><line x1="'+pd+'" y1="'+(H-pd)+'" x2="'+(W-pd)+'" y2="'+(H-pd)+'" stroke="#ddd"/><line x1="'+pd+'" y1="'+pd+'" x2="'+pd+'" y2="'+(H-pd)+'" stroke="#ddd"/>';
  window._dsScatter={pts:pts,xl:VARS[xk],yl:VARS[yk]};
  g+=pts.map(function(a,i){return '<circle cx="'+sx(a.x).toFixed(1)+'" cy="'+sy(a.y).toFixed(1)+'" r="3.8" fill="'+_dstat.dcol(a.k)+'" opacity=".72" onmouseenter="dsTip(event,'+i+')" onmousemove="dsTipAt(event)" onmouseleave="dsTipOff()" onclick="dsTip(event,'+i+')"></circle>';}).join('');
  ...
  box.innerHTML=g+'<div id="dsTip" class="dstip" style="display:none"></div>';
  const R=r.r||0;document.getElementById('dsReg').innerHTML='· <b>R² '+r.r2.toFixed(2)+'</b> · r '+R.toFixed(2)+' · n='+r.n;
}
function dsTip(e,i){var t=document.getElementById('dsTip'),S=window._dsScatter;if(!t||!S)return;var a=S.pts[i];t.innerHTML='<b>'+a.nm+'</b><br>'+S.xl+' <b>'+(+a.x).toFixed(1)+'</b> · '+S.yl+' <b>'+(+a.y).toFixed(1)+'</b>';t.style.display='block';dsTipAt(e);}
...
function printDiagReport(){
  ...
  var date='';try{date=new Date().toLocaleDateString('ko-KR');}catch(e){}
  var esc=function(x){return String(x).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');};
  var gc=g.dongs.reduce(function(m,p){var gr=gradeOf(p);m[gr]=(m[gr]||0)+1;return m;},{});
  var gbar=['S','A','B','C','D'].filter(function(x){return gc[x];}).map(function(x){return '<span class=gchip style="background:'+GC[x]+'22;color:'+GC[x]+'">'+x+'등급 '+gc[x]+'</span>';}).join('');
  var dev={};g.rankDom.forEach(function(x){dev[x[0]]=x[1];});
  var maxAbs=Math.max.apply(null,[1].concat(g.rankDom.map(function(x){return Math.abs(x[1]);})));
  var prof=DKEYS.map(function(d){var v=dev[d]||0,w=(Math.abs(v)/maxAbs*50).toFixed(1);
    var seg=v<0?'<span class=pseg style="right:50%;width:'+w+'%;background:#b0603f"></span>':'<span class=pseg style="left:50%;width:'+w+'%;background:#2f6b4e"></span>';
    return '<div class=prow><span class=plabel>'+DOMINFO[d][0]+' '+SHORT[d]+'</span><span class=pbar>'+seg+'</span><span class=pval style="color:'+(v<0?'#b0603f':'#2f6b4e')+'">'+(v>=0?'+':'')+Math.round(v)+'</span></div>';}).join('');
  var weak=g.rankDom.slice(0,3),strong=g.rankDom.slice(-2).reverse();
  var wr='<tr><th>도메인</th><th>전국 지자체 평균 대비</th></tr>'+weak.map(function(w){return '<tr><td>'+METRICS[w[0]]+'</td><td style="color:#b0603f;font-weight:700">'+(w[1]>=0?'+':'')+Math.round(w[1])+'</td></tr>';}).join('');
  var byd={};g.blind.forEach(function(b){(byd[b.d]=byd[b.d]||[]).push(b.p.adm_nm);});
  var prio=Object.keys(byd).map(function(d){return [d,byd[d]];}).sort(function(a,b){return b[1].length-a[1].length;}).map(function(e){return METRICS[e[0]]+' '+e[1].length+'곳';}).join(' · ');
  var bl=g.blindN?('<table><tr><th>동</th><th>인구</th><th>취약 도메인</th><th>점수(백분위)</th></tr>'+g.blind.map(function(b){return '<tr><td>'+esc(b.p.adm_nm)+'</td><td>'+nf(b.p.pop_total)+'명</td><td>'+METRICS[b.d]+'</td><td style="color:#b0603f;font-weight:700">'+Math.round(b.v)+'</td></tr>';}).join('')+'</table><p class=hi>&rarr; 우선 보강 대상: '+esc(prio)+'</p>'):'<p>인구 1만+ 동에서 하위 20% 도메인 없음 — 사각지대 없음.</p>';
  var st=strong.map(function(w){return '<li>'+METRICS[w[0]]+' <b style="color:#2f6b4e">+'+Math.round(w[1])+'</b></li>';}).join('');
  ...
  var html='<!doctype html><html lang=ko><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>'+esc(g.sido+' '+g.sgg)+' 생활여건 진단 리포트</title>'
   ...
   +'<h2>사각지대 동 <span class=mut style="font-weight:400">인구 1만+ 인데 특정 도메인 전국 하위 20% · 총 '+g.blindN+'건</span></h2>'+bl
   +'<h2>상대 강점 도메인</h2><ul class=sum>'+(st||'<li>—</li>')+'</ul>'
   +'<div class=foot>방법론 · 각 지표를 시설밀도(인구 1만명당·면적 ㎢당 혼합)와 근접성의 백분위로 결합 → 도메인 가중평균 → 종합지수. 사각지대 = 인구 1만+ 인데 해당 도메인 전국 하위 20%. <b>점수는 전국 읍면동 상대평가(백분위)로 참고용</b>이며 절대 적정성이 아닙니다. 종합지수는 인구밀도와 상관이 높아 도시성을 일부 반영하므로 순위 비교보다 <b>지역 내부의 상대 취약(사각지대) 진단</b>에 활용을 권합니다. 복지·돌봄은 지오코딩 약 96% 커버. 데이터: 공공데이터포털·SGIS(2025 2Q)·심평원·소상공인·국토부 실거래가 · 데모 https://lucestdail.github.io/nli/</div>'
   +'</body></html>';
  var w=window.open('','_blank');if(w){w.document.write(html);w.document.close();}else alert('팝업이 차단되었습니다. 팝업 허용 후 다시 시도하세요.');
}
function diagToMap(){
