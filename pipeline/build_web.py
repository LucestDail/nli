 ...
 *{box-sizing:border-box}
 ...
 .findlist{display:flex;flex-direction:column;gap:5px;max-height:215px;overflow:auto}
...
document.getElementById('foot1').innerHTML='데이터 출처 · 공공데이터포털 표준데이터 · SGIS 경계·인구(2025 2분기) · 건강보험심사평가원(2026.6) · 소상공인시장진흥공단 · 국토교통부 아파트 실거래가 · safetydata.go.kr · VWorld 지오코딩<br>방법 · 시설밀도(인구 1만명당·면적 ㎢당 혼합)와 근접성의 백분위 결합 → 도메인 가중평균, 도농 코호트·인구가중 중심점 보정<br>9개 도메인 32개 지표 읍면동 정밀(복지는 지오코딩 약 96% 커버) · 점수는 전국 읍면동 상대평가(백분위)로 <b>참고용</b> — 시설 밀도·근접 기반이라 도시성(인구밀도)을 일부 반영하므로 <b>동일 도농유형 내 상대비교</b>를 함께 보세요 · <a onclick="openMethod()" style="cursor:pointer;color:var(--ocean);font-weight:700">방법론·한계 자세히 →</a><br><a href="mailto:lucestdail@kakao.com?subject=%5B%EB%8F%99%EB%84%A4%EC%82%B4%EA%B8%B0%EC%A7%80%EC%88%98%5D%20%EB%AC%B8%EC%9D%98" style="display:inline-block;margin-top:9px;color:var(--ocean);font-weight:700;text-decoration:none">문의</a>';
