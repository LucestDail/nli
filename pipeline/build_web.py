...
<div id="app">
 ...
 <div id="cmpPanel">
   <div class="cmphd"><b>⚖️ 지역 비교 <span id="cmpCount" class="muted" style="font-weight:400"></span></b>
 ...
 <div id="diagStatModal" class="rmodal" style="display:none" onclick="if(event.target===this)closeDiagStat()"><div class="rmbox" style="width:660px;max-width:100%;max-height:88vh">
 ...
 <div class="view map" id="v-map" style="display:flex">
  ...
  <div class="view" id="v-compare" style="display:none"><div class="wrap inswrap">
   <div class="card" style="background:linear-gradient(135deg,#eef4f1,#eaf1f4);border:0">
     ...
     <div class="muted" style="font-size:14.5px;line-height:1.8;max-width:820px">
       ...
       아래에서 <b style="color:var(--ocean)">지자체 진단 → 통계 분석</b>을 확인하고, 관심 동네는 <b>⊕</b>로 담아 <b style="color:var(--ocean)">레이더로 비교</b>하세요.
     </div>
   </div>
 </div></div>
 ...
 <div class="view" id="v-diag" style="display:none"><div class="wrap inswrap">
   ...
   <div class="sub" style="margin-bottom:12px">시군구 단위 <b>취약 도메인</b>·<b>사각지대 동</b>(인구 1만+ 하위 20%) 진단 · <b>좌측 표에서 지자체를 고르면 우측 리포트가 갱신</b>됩니다. ⊕로 여러 곳을 담아 비교하거나 <b>통계 분석</b>을 보세요.</div>
...
function renderInsight(){renderDiag();renderStats();}
function showTab(v){
...
function diagCardHTML(g,rankOf,total){
  ...
  return `<div class="flex" style="justify-content:space-between;align-items:flex-start;gap:12px">
    ...
    <div class="flex" style="gap:6px;flex-shrink:0;justify-content:flex-end"><button class="btn" onclick="diagToMap()">🗺 지도</button><button class="btn on" onclick="openDiagStat()">📊 통계 분석</button><span class="dltools flex" style="gap:6px"><button class="btn" onclick="copyDiagReport(this)">📋 복사</button><button class="btn" onclick="downloadDiagCSV()">⬇ CSV</button><button class="btn" onclick="printDiagReport()">🖨 인쇄</button></span></div></div>
   <div class="dkpi">
...
function openDiagStat(){
  ...
  const prof=DKEYS.map(function(dd){const dv=Math.round((g.rankDom.find(x=>x[0]===dd)||[0,0])[1]),w=Math.min(50,Math.abs(dv))/50*100;
    return '<div style="display:flex;align-items:center;gap:8px;margin:5px 0;font-size:12.5px"><span style="width:92px;text-align:right;flex-shrink:0">'+DOMINFO[dd][0]+' '+SHORT[dd]+'</span>'
     +'<div style="flex:1;display:flex;align-items:center"><div style="flex:1;text-align:right"><span style="display:inline-block;height:12px;border-radius:3px;background:#b0603f;width:'+(dv<0?w:0)+'%"></span></div><div style="width:1px;height:16px;background:#d8cdb8"></div><div style="flex:1"><span style="display:inline-block;height:12px;border-radius:3px;background:#2f6b4e;width:'+(dv>=0?w:0)+'%"></span></div></div>'
     +'<span style="width:34px;font-weight:700;flex-shrink:0;color:'+(dv<0?'#b0603f':'#2f6b4e')+'">'+(dv>=0?'+':'')+dv+'</span></div>';}).join('');
  const bl=g.blind.slice(0,10).map(function(b){return '<tr><td style="text-align:left">'+b.p.adm_nm+'</td><td class="n">'+nf(b.p.pop_total)+'명</td><td style="text-align:left">'+METRICS[b.d]+'</td><td class="n" style="color:#b0603f;font-weight:700">'+Math.round(b.v)+'</td></tr>';}).join('');
  document.getElementById('diagStatBody').innerHTML=
    '<div class="flex" style="justify-content:space-between;align-items:baseline;gap:10px"><h2 style="margin:0">'+g.sido+' '+g.sgg+'</h2><div class="muted">전국 취약순위 <b style="color:var(--terra)">'+diagCur.rank+'</b>/'+diagCur.total+'</div></div>'
    +'<div class="dkpi" style="margin-top:12px"><div class="kpi"><b style="color:'+color(g.nli)+'">'+g.nli.toFixed(1)+'</b><span>평균 종합지수 · 백분위</span></div><div class="kpi"><b style="color:var(--terra)">'+g.blindN+'</b><span>사각지대 · 동×도메인</span></div><div class="kpi"><b>'+g.dongs.length+'</b><span>관할 동</span></div><div class="kpi"><b>'+nf(g.pop)+'</b><span>인구(명)</span></div></div>'
    +'<h3 style="margin:18px 0 6px">도메인 프로파일 <span class="muted" style="font-weight:400;font-size:13px">전국 지자체 평균(중앙선) 대비 · 좌 취약 / 우 강점</span></h3>'+prof
    +'<h3 style="margin:18px 0 6px">등급 분포 <span class="muted" style="font-weight:400;font-size:13px">관할 '+g.dongs.length+'개 동</span></h3><div style="font-size:14px">'+(P.gdist||'—')+'</div>'
    +'<h3 style="margin:18px 0 6px">🎯 사각지대 동 <span class="muted" style="font-weight:400;font-size:13px">'+g.blindN+'건 · 인구 많은 순 상위 10</span></h3>'
    +(g.blindN?'<div style="overflow:auto"><table style="font-size:12.5px"><tr><th style="text-align:left">동</th><th>인구</th><th style="text-align:left">취약 도메인</th><th>점수</th></tr>'+bl+'</table></div>':'<div class="muted">인구 1만+ 동에서 하위 20% 도메인 없음 — 사각지대 없음.</div>')
    +'<div class="muted" style="font-size:11px;margin-top:14px">점수는 전국 읍면동 상대평가(백분위) 기준. 데이터: 공공데이터포털·SGIS·심평원 등.</div>';
  document.getElementById('diagStatModal').style.display='flex';
}
function downloadDiagCSV(){
  if(!diagCur)return;const g=diagCur.g;
  if(!g.blindN){alert('사각지대 없음 — 다운로드할 데이터가 없습니다');return;}
  const rows=[['시도','시군구','동','인구','취약도메인','점수(백분위)']];
  g.blind.forEach(b=>rows.push([g.sido,g.sgg,b.p.adm_nm,b.p.pop_total||0,METRICS[b.d],Math.round(b.v)]));
  const csv='﻿'+rows.map(r=>r.map(c=>'"'+String(c).replace(/"/g,'""')+'"').join(',')).join('\r\n');
  const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));
  a.download='사각지대_'+g.sgg+'.csv';a.click();URL.revokeObjectURL(a.href);
}
function toggleDiagCmp(k){const i=diagCmp.indexOf(k);if(i>=0)diagCmp.splice(i,1);else{if(diagCmp.length>=4){alert('최대 4곳까지 비교할 수 있어요');return}diagCmp.push(k)}renderDiag();}
function renderDiagCmp(rows){
  const el=document.getElementById('diagCmp');if(!el)return;
  if(diagCmp.length<2){el.style.display='none';el.innerHTML='';return}
  el.style.display='block';
  const gs=diagCmp.map(k=>rows.find(r=>r.key===k)).filter(Boolean);
  const sido=s=>s.replace('특별자치도','').replace('특별자치시','').replace('광역시','').replace('특별시','').replace('자치도','');
  let h='<div class="card" style="margin-bottom:16px"><div class="flex" style="justify-content:space-between;align-items:center"><h3 style="margin:0">지자체 비교 <span class="muted" style="font-weight:400;font-size:13px">'+gs.length+'곳 · 도메인은 전국 평균 대비 편차</span></h3><button class="btn" onclick="diagCmp=[];renderDiag()">비우기</button></div>';
  h+='<div style="overflow:auto"><table style="margin-top:12px;min-width:'+(140+gs.length*110)+'px"><tr><th style="text-align:left">항목</th>'+gs.map(g=>'<th>'+g.sgg+'<br><span class="muted" style="font-weight:400">'+sido(g.sido)+'</span></th>').join('')+'</tr>';
  h+='<tr><td style="text-align:left">평균 종합지수</td>'+gs.map(g=>'<td style="text-align:center"><b style="color:'+color(g.nli)+'">'+g.nli.toFixed(1)+'</b></td>').join('')+'</tr>';
  h+='<tr><td style="text-align:left">사각지대 (동×도메인)</td>'+gs.map(g=>'<td style="text-align:center;color:var(--terra);font-weight:700">'+g.blindN+'</td>').join('')+'</tr>';
  DKEYS.forEach(d=>{h+='<tr><td style="text-align:left">'+DOMINFO[d][0]+' '+SHORT[d]+'</td>'+gs.map(g=>{const dv=Math.round((g.rankDom.find(x=>x[0]===d)||[0,0])[1]);return '<td style="text-align:center;color:'+(dv<0?'#b0603f':'#2f6b4e')+'">'+(dv>=0?'+':'')+dv+'</td>'}).join('')+'</tr>';});
  h+='</table></div></div>';
  el.innerHTML=h;
}
function diagToMap(){
...
function printDiagReport(){
  ...
  // 9도메인 다이버징 프로파일(전국 평균=중앙, 좌 빨강/우 초록)
  const dev={};g.rankDom.forEach(x=>dev[x[0]]=x[1]);
  const maxAbs=Math.max(1,...g.rankDom.map(x=>Math.abs(x[1])));
  const prof=DKEYS.map(d=>{const v=dev[d]||0,w=(Math.abs(v)/maxAbs*50).toFixed(1);
    const seg=v<0?`<span class=pseg style="right:50%;width:${w}%;background:#b0603f"></span>`:`<span class=pseg style="left:50%;width:${w}%;background:#2f6b4e"></span>`;
  ...
  const gc=g.dongs.reduce((m,p)=>{const gr=gradeOf(p);m[gr]=(m[gr]||0)+1;return m},{});
  const gbar=['S','A','B','C','D'].filter(x=>gc[x]).map(x=>`<span class=gchip style="background:${GC[x]}22;color:${GC[x]}">${x}등급 ${gc[x]}</span>`).join('');
  const wr=`<tr><th>도메인</th><th>전국 대비 편차</th></tr>`+P.weak.map(w=>`<tr><td>${METRICS[w[0]]}</td><td style="color:#b0603f;font-weight:700">${w[1]>=0?'+':''}${Math.round(w[1])}</td></tr>`).join('');
  const bl=g.blindN?`<table><tr><th>동</th><th>인구</th><th>취약 도메인</th><th>점수(백분위)</th></tr>`+g.blind.map(b=>`<tr><td>${esc(b.p.adm_nm)}</td><td>${nf(b.p.pop_total)}명</td><td>${METRICS[b.d]}</td><td style="color:#b0603f;font-weight:700">${Math.round(b.v)}</td></tr>`).join('')+`</table><p class=hi>→ 우선 보강: ${esc(P.prio)}.</p>`:'<p>인구 1만+ 동에서 하위 20% 도메인 없음 — 사각지대 없음.</p>';
  const st=P.strong.map(w=>`<li>${METRICS[w[0]]} <b style="color:#2f6b4e">+${Math.round(w[1])}</b></li>`).join('');
  const one=P.weak[0][1]<0?`<b>1순위 보강 대상: ${METRICS[P.weak[0][0]]}</b> — 전국 지자체 평균보다 <b>${Math.abs(Math.round(P.weak[0][1]))}p</b> 낮습니다.`:'모든 도메인이 전국 지자체 평균 이상입니다.';
  const html=`<!doctype html><html lang=ko><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>${esc(g.sido+' '+g.sgg)} 생활여건 진단 리포트</title>
...
<p class=mut>취약 도메인·사각지대 진단 · 전국 읍면동 9개 도메인 32개 지표 상대평가(백분위) 기준</p>
<div class=stats><div class=stat><b>${g.nli.toFixed(1)}</b><span>평균 종합지수</span></div><div class=stat><b>${diagCur.rank}/${diagCur.total}</b><span>전국 취약순위</span></div><div class=stat><b>${g.dongs.length}개</b><span>관할 동</span></div><div class=stat><b>${nf(g.pop)}</b><span>인구(명)</span></div></div>
<h2>도메인 프로파일 <span class=mut style="font-weight:400">— 전국 지자체 평균(중앙선) 대비 · 좌 취약 / 우 강점</span></h2>
...
<div class=foot>방법론 · 각 지표를 시설밀도(인구 1만명당·면적 ㎢당 혼합)와 근접성의 백분위로 결합 → 도메인 가중평균 → 종합지수. 사각지대 = 인구 1만+ 인데 해당 도메인 전국 하위 20%. 점수는 전국 읍면동 상대평가(백분위)로 참고용이며 복지·돌봄은 지오코딩 약 89% 커버. · 데이터: 공공데이터포털·SGIS(2025 2Q)·심평원·소상공인·국토부 실거래가 · 데모 https://lucestdail.github.io/nli/</div>
</body></html>`;
