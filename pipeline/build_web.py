 ...
 *{box-sizing:border-box}
 ...
 .statgrid .sc{flex:1 1 320px;display:flex;flex-direction:column;height:400px;background:#faf8f4;border:1px solid var(--line);border-radius:14px;padding:15px 16px;min-width:0;overflow:hidden}
 .statgrid .sc.wide{flex:2 1 520px}
 .statgrid .sc.full,.statgrid .sc.span2{flex-basis:100%}
 .scb{flex:1 1 auto;overflow:auto;min-height:0}
 ...
 .scrollv{border:1px solid var(--line2);border-radius:10px}
 ...
 @media(max-width:760px){.statgrid .sc{flex-basis:100%;height:auto;max-height:74vh}}
...
function initMap(){
  ...
  layer=L.geoJSON(DATA,{style:mStyle,onEachFeature:(f,l)=>{f.properties._l=l;l.bindTooltip(()=>dongTip(f.properties),{sticky:true,direction:'top',className:'dtip',opacity:1});l.on('mouseover',()=>l.setStyle({weight:1.6,color:'#16232e'}));l.on('mouseout',()=>layer.resetStyle(l));l.on('click',()=>showDetail(f.properties))}}).addTo(map);
