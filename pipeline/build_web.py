...
<div id="app">
 ...
 <div class="view" id="v-diag" style="display:none"><div class="wrap inswrap">
   ...
   <div class="flex" style="margin-bottom:12px">
     ...
     <select id="diagSort"><option value="nli">취약(평균지수 낮은) 순</option><option value="nli_desc">우수(평균지수 높은) 순</option><option value="blind">사각지대 많은 순</option><option value="blind_asc">사각지대 적은 순</option><option value="pop">인구 많은 순</option><option value="dongs">관할 동 많은 순</option></select><button class="btn on" onclick="openDiagStat()">📊 통계 분석</button>
...
function openDiagStat(){
  ...
  if(!gs.length){alert('표에서 지자체를 선택(행 클릭)하거나 ⊕로 담으세요.');return;}
  const natRank=[...rows].sort((a,b)=>a.nli-b.nli),rankOf=new Map();natRank.forEach((r,i)=>rankOf.set(r.key,i+1));
  ...
  t+='<tr><td style="text-align:left">사각지대(동×도메인)</td>'+gs.map(g=>'<td class="n" style="color:var(--terra);font-weight:700">'+g.blindN+'</td>').join('')+'</tr>';
  t+='<tr><td style="text-align:left">관할 동 / 인구</td>'+gs.map(g=>'<td class="n">'+g.dongs.length+'개 / '+nf(g.pop)+'</td>').join('')+'</tr>';
  ...
  t+='</table></div>';
  let bl='';
  gs.forEach(function(g){bl+='<div style="margin-top:14px"><div class="fld" style="font-size:11.5px">🎯 '+g.sgg+' 사각지대 동 <span class="muted" style="font-weight:400;text-transform:none;letter-spacing:0">'+g.blindN+'건</span></div>';
    bl+=g.blindN?('<div style="overflow:auto"><table style="font-size:12px;margin-top:4px"><tr><th style="text-align:left">동</th><th>인구</th><th style="text-align:left">취약</th><th>점수</th></tr>'+g.blind.slice(0,5).map(b=>'<tr><td style="text-align:left">'+b.p.adm_nm+'</td><td class="n">'+nf(b.p.pop_total)+'</td><td style="text-align:left">'+METRICS[b.d]+'</td><td class="n" style="color:#b0603f;font-weight:700">'+Math.round(b.v)+'</td></tr>').join('')+'</table></div>'):'<div class="muted" style="font-size:12px">사각지대 없음</div>';
    bl+='</div>';});
  document.getElementById('diagStatBody').innerHTML=
    '<div class="flex" style="justify-content:space-between;align-items:baseline;gap:10px"><h2 style="margin:0">'+(single?gs[0].sido+' '+gs[0].sgg:gs.length+'개 지자체 통계 분석')+'</h2><div class="muted">도메인=전국 평균 대비 편차 · 각 행 최고 초록</div></div>'
    +'<div style="margin-top:14px">'+t+'</div>'+bl
    +'<div class="muted" style="font-size:11px;margin-top:14px">점수는 전국 읍면동 상대평가(백분위). 도메인 값은 전국 지자체 평균 대비 편차.</div>';
...
function renderDiagCmp(rows){
  ...
  let h='<div class="card" style="margin-bottom:16px"><div class="flex" style="justify-content:space-between;align-items:center"><h3 style="margin:0">지자체 비교 <span class="muted" style="font-weight:400;font-size:13px">'+gs.length+'곳 · 도메인은 전국 평균 대비 편차 · <span style="cursor:pointer;color:var(--ocean)" onclick="openDiagStat()">📊 통계 분석</span></span></h3><button class="btn" onclick="diagCmp=[];renderDiag()">비우기</button></div>';
