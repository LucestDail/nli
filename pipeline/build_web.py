...
<div id="app">
 ...
 <div class="view" id="v-diag" style="display:none"><div class="wrap">
   ...
   <div id="diagCmp" style="display:none"></div>
...
function diagReportMD(g,rank,total){
  ...
  L.push('','---',`방법론: 각 지표를 밀도(인구·면적)·근접성 백분위로 혼합 → 도메인 가중평균 → 종합지수. 점수는 전국 읍면동 상대평가(백분위)로 참고용. 복지·돌봄은 지오코딩 약 88% 커버.`);
  return L.join('\n');
}
...
<div class=foot>방법론 · 각 지표를 시설밀도(인구 1만명당·면적 ㎢당 혼합)와 근접성의 백분위로 결합 → 도메인 가중평균 → 종합지수. 사각지대 = 인구 1만+ 인데 해당 도메인 전국 하위 20%. 점수는 전국 읍면동 상대평가(백분위)로 참고용이며 복지·돌봄은 지오코딩 약 88% 커버. · 데이터: 공공데이터포털·SGIS(2025 2Q)·심평원·소상공인·국토부 실거래가 · 데모 https://lucestdail.github.io/nli/</div>
</body></html>`;
